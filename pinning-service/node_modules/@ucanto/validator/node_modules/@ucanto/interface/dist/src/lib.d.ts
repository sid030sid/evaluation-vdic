...
export type { Transport, Phantom, Tuple, DID, Signature, SignatureView, ByteView, Capabilities, Capability, Fact, UCANBlock, UCANLink, Link, Link as IPLDLink, IPLDBlock, Block, Ability, Resource, SigAlg, MultihashDigest, MultihashHasher, MultibaseDecoder, MultibaseEncoder, MulticodecCode, Principal, ToJSON, ToString, UnknownLink, JSONUnknown, Crypto, };
...
export interface BuildOptions<T extends unknown = unknown, C extends MulticodecCode = MulticodecCode, A extends MulticodecCode = MulticodecCode> {
 ...
 */
export interface Delegation<C extends Capabilities = Capabilities> extends IPLDView<UCAN.UCAN<C>>, IPLDViewBuilder<Delegation<C>> {
 ...
 */
export type Allows<URI extends Resource = Resource, Abilities extends ResourceAbilities = ResourceAbilities> = {
    [K in URI]: Abilities;
};
/**
 ...
 */
export type ResourceAbilities<Can extends Ability = any, Caveats extends Record<string, unknown> = Record<string, unknown>> = {
    [K in Can]: Caveats[];
};
/**
 ...
 */
export type JoinAbilities<T extends ResourceAbilities, U extends ResourceAbilities> = {
...
export type JoinAllows<T extends Allows, U extends Allows> = {
    [K in keyof T | keyof U]: JoinAbilities<K extends keyof T ? (T[K] extends ResourceAbilities ? T[K] : {}) : {}, K extends keyof U ? (U[K] extends ResourceAbilities ? U[K] : {}) : {}>;
};
/**
 * Utility type that infers set of capabilities delegated by one or more
 * {@link Delegation}s in UCAN 0.10 format.
 */
export type InferAllowedFromDelegations<T extends [unknown, ...unknown[]]> = T extends [infer A] ? InferAllowedFromDelegation<A> : T extends [infer A, infer B] ? JoinAllows<InferAllowedFromDelegation<A>, InferAllowedFromDelegation<B>> : T extends [infer A, infer B, ...infer Rest] ? JoinAllows<InferAllowedFromDelegation<A>, InferAllowedFromDelegations<[B, ...Rest]>> : never;
/**
 * Utility type that infers set of capabilities delegated by a single
 * {@link Delegation}
 */
export type InferAllowedFromDelegation<T> = T extends Delegation<infer Capabilities> ? InferAllowedFromCapabilities<Capabilities> : never;
/**
 * Utility type that infers set of capabilities in UCAN 0.10 format from a
 * {@link Capability} tuple.
 */
export type InferAllowedFromCapabilities<T> = T extends [infer A] ? InferAllowedFromCapability<A> : T extends [infer A, ...infer Rest] ? JoinAllows<InferAllowedFromCapability<A>, InferAllowedFromCapabilities<Rest>> : never;
/**
 * Utility type that infers set of capabilities in UCAN 0.10 format from a
 * single {@link Capability}.
 */
export type InferAllowedFromCapability<T> = T extends Capability<infer Can, infer URI, infer Caveats> ? {
    [K in URI]: {
        [K in Can]: (Caveats & {})[];
    };
} : never;
 ...
 */
export interface OutcomeModel<Ok extends {} = {}, Error extends {} = {}, Ran extends Invocation = Invocation> {
 ...
 */
export interface ReceiptModel<Ok extends {} = {}, Error extends {} = {}, Ran extends Invocation = Invocation> {
 ...
 */
export interface Receipt<Ok extends {} = {}, Error extends {} = {}, Ran extends Invocation = Invocation, Alg extends SigAlg = SigAlg> extends IPLDView<ReceiptModel<Ok, Error, Ran>>, IPLDViewBuilder<Receipt<Ok, Error, Ran, Alg>> {
...
export interface InstructionModel<Op extends Ability = Ability, URI extends Resource = Resource, Input extends Record<string, unknown> = Record<string, unknown>> {
    op: Op;
    rsc: URI;
    input: Input;
    nnc: string;
...
export interface IssuedInvocation<C extends Capability = Capability> extends IPLDViewBuilder<Invocation<C>> {
    readonly issuer: Principal;
...
export type InferInvocations<T extends Tuple> = T extends [
    ...
    infer Next,
    ...infer Rest
] ? [Invocation<C>, ...InferInvocations<[Next, ...Rest]>] : T extends [ServiceInvocation<infer C>] ? [Invocation<C>] : never;
/**
 ...
 */
export interface ServiceMethod<I extends Capability, O extends {}, X extends Failure> {
...
export type InferReceipt<C extends Capability, S extends Record<string, any>> = ResolveServiceMethod<S, C['can']> extends ServiceMethod<infer _, infer T, infer X> ? Receipt<T & {}, X | HandlerNotFound | HandlerExecutionError | InvalidAudience | Unauthorized> : never;
...
export type InferReceipts<I extends unknown[], T extends Record<string, any>> = I extends [] ? [] : I extends [ServiceInvocation<infer C, T>, ...infer Rest] ? [InferReceipt<C, T>, ...InferReceipts<Rest, T>] : never;
/**
...
export interface AgentMessageBuilder<T> extends IPLDViewBuilder<AgentMessage<T>> {
}
export interface AgentMessage<T = unknown> extends IPLDView<AgentMessageModel<T>> {
...
export interface ReportModel<T = unknown> extends Phantom<T> {
    receipts: Record<ToString<Link<ReceiptModel['ocm']['ran']>>, Link<ReceiptModel>>;
}
export interface ReportBuilder<T> extends IPLDViewBuilder<IPLDView<ReportModel<T>>> {
...
export interface IssuedInvocationView<C extends Capability = Capability> extends IssuedInvocation<C> {
    ...
    execute<T extends InvocationService<C>>(service: ConnectionView<T>): Await<InferReceipt<C, T>>;
}
...
export interface OutboundCodec extends Transport.RequestEncoder, Transport.ResponseDecoder {
}
/** @deprecated */
export interface OutboundTransportOptions extends OutboundCodec {
}
export interface ConnectionOptions<T extends Record<string, any>> extends Transport.EncodeOptions {
    /**
...
export interface ConnectionView<T extends Record<string, any>> extends Connection<T> {
    id: Principal;
    execute<C extends Capability, I extends Transport.Tuple<ServiceInvocation<C, T>>>(...invocations: I): Await<InferReceipts<I, T>>;
}
...
export interface ServerView<T extends Record<string, any>> extends Server<T>, Transport.Channel<T> {
    ...
    run<C extends Capability>(invocation: ServiceInvocation<C, T>): Await<InferReceipt<C, T>>;
}
/**
 ...
 */
export interface Signer<ID extends DID = DID, Alg extends SigAlg = SigAlg> extends UCAN.Signer<ID, Alg>, Verifier<ID, Alg> {
    /**
 ...
 */
export interface Verifier<ID extends DID = DID, Alg extends SigAlg = SigAlg> extends UCAN.Verifier<ID, Alg> {
    /**
...
export type InferInvokedCapability<C extends CapabilityParser<Match<ParsedCapability>>> = C extends CapabilityParser<Match<infer T>> ? InferCapability<T & Capability> : never;
